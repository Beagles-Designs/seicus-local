...
if ( !function_exists('pagelist_unqprfx_add_stylesheet') ) {
	function pagelist_unqprfx_add_stylesheet() {
		wp_enqueue_style( 'page-list-style', plugins_url( '/css/page-list.css', __FILE__ ), false, '4.4', 'all' );
	}
...
if ( ! function_exists('pagelist_unqprfx_plugin_meta') ) {
	function pagelist_unqprfx_plugin_meta( $links, $file ) { // add links to plugin meta row
		if ( $file == plugin_basename( __FILE__ ) ) {
			$row_meta = array(
				'support' => '<a href="http://web-profile.net/wordpress/plugins/page-list/" target="_blank">Page-list</a>',
				'donate' => '<a href="http://web-profile.net/donate/" target="_blank">Donate</a>',
				'pro' => '<a href="https://1.envato.market/70ANO" target="_blank" title="Advanced iFrame Pro">Advanced iFrame Pro</a>'
			);
...
if ( ! function_exists('pagelist_unqprfx_admin_notice') ) {
    function pagelist_unqprfx_admin_notice() {
        $query_action = 'install-plugin';
        $plugin_slug = 'page-list';
        $plugin_install_url = wp_nonce_url(
            ...
            $query_action.'_'.$plugin_slug
        );

        echo '<div class="notice notice-error is-dismissible">
        <p>The Sitemap plugin is going to be discontinued soon and will not receive security updates. 
        The code and functionality migrated to the <a href="https://wordpress.org/plugins/page-list/" target="_blank">Page-List plugin</a>.</p>
        <p>Please consider performing the migration from the Sitemap to the Page-List plugin with these few simple steps:</p>
        <ol>
        <li><a href="'.esc_url( $plugin_install_url ).'">Install and activate the Page-List plugin</a></li>
        <li>Deactivate and delete the Sitemap plugin</li>
        </ol>
        <p>That is it. All the functionality should work the same after the migration.</p>
        </div>';
    }
    add_action('admin_notices', 'pagelist_unqprfx_admin_notice');
}